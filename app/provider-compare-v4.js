(() => {
  ...
  const COMPARISON_ADAPTERS = Object.freeze({
    broadway: Object.freeze({ normalizeSession: normalizeBroadwaySession }),
    mcl: Object.freeze({
      fetchShows: fetchMCLShows,
      normalizeSession: normalizeMCLSession,
      metadataComplete(successes) { return successes.every(item => item.result?.metadataComplete === true); },
      canReuse(result) { return result?.metadataComplete === true; }
    }),
    emperor: Object.freeze({ normalizeSession: normalizeEmperorSession })
  });
  ...
  function comparisonAdapter(provider) {
    ...
    return builtIn || runtime ? { ...(builtIn || {}), ...(runtime || {}) } : null;
  }
  ...
  function normalizeSourceId(provider, value) {
    return sharedCore?.normalizeSourceId?.(provider, value) || String(value || "").replace(new RegExp(`^${provider}:`), "").trim();
  }
  function getMatch(matchId) { return window.HKCinemaProviderMatches?.get?.(matchId) || null; }
  ...
  function activeProviders(match = state.match) { return PROVIDERS.filter(provider => providerSourceIds(provider.key, match).length > 0); }
  function providerLabels(match = state.match) { return activeProviders(match).map(provider => provider.label); }
  function metadataCore() { return window.HKCinemaShowtimeMetadata || null; }
  function sessionMetadata(session) {
    return metadataCore()?.normalizeSession?.(session) || {
      languages: ["unknown"], subtitles: ["unknown"], formats: ["unknown"],
      languageLabels: ["語言未提供"], subtitleLabels: ["字幕未提供"], formatLabels: []
    };
  }
  ...
  function filteredRawSessions(provider, sessions, match = state.match) { return (sessions || []).filter(session => rawSessionMatches(provider, session, match)); }
  function uniqueDates(values) {
    return Array.from(new Set((values || []).map(value => String(value || "").slice(0, 10)).filter(value => /^\d{4}-\d{2}-\d{2}$/.test(value)))).sort();
  }
  function availableDatesFor(provider, result, match = state.match) {
    ...
    if (!usesSessionCriteria(provider, match) || !Array.isArray(result?.allSessions)) return uniqueDates(result?.availableDates || []);
    return uniqueDates(metadataCore()?.candidateDatesForCriteria?.(result, match.sessionCriteria, state.criteriaDateDecisions[provider]) || result?.availableDates || []);
  }
  ...
  function formatDate(dateString) {
    ...
    return new Intl.DateTimeFormat("zh-HK", { timeZone: "Asia/Hong_Kong", month: "numeric", day: "numeric", weekday: "short" }).format(date);
  }
  function errorMessage(error) { return error instanceof Error ? error.message : String(error || "未知錯誤"); }
  function combinedDates() { return uniqueDates(activeProviders().flatMap(provider => state.availableDates[provider.key] || [])); }
  function firstPreferredDate() {
    const dates = combinedDates();
    if (!dates.length) return null;
    return dates.map(date => ({ date, count: activeProviders().filter(provider => state.availableDates[provider.key].includes(date)).length }))
  ...
  function dateProviders(date) {
    ...
    if (available.length === activeCount && activeCount > 1) label = sharedCore?.allProviderLabel?.(activeCount) || `${activeCount} 院線`;
    if (!label) label = "暫無院線";
  ...
  function childController(parentSignal, timeoutMs) {
    const controller = new AbortController();
    let timedOut = false;
    const onParentAbort = () => { try { controller.abort(parentSignal?.reason || "lifecycle"); } catch { controller.abort(); } };
    if (parentSignal?.aborted) onParentAbort();
    else parentSignal?.addEventListener?.("abort", onParentAbort, { once: true });
    const timer = setTimeout(() => { timedOut = true; try { controller.abort("timeout"); } catch { controller.abort(); } }, timeoutMs);
    return { controller, timedOut: () => timedOut, cleanup() { clearTimeout(timer); parentSignal?.removeEventListener?.("abort", onParentAbort); } };
  }
  ...
  async function fetchWorkerShows(provider, sourceId, date, parentSignal) {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/${provider}/movies/${encodeURIComponent(sourceId)}/shows${query}`, { cache: "no-store", signal: lifecycle.controller.signal });
      if (response.status === 404) return { availableDates: [], selectedDate: date, sessions: [], _health: { updatedAt: new Date().toISOString(), source: "network" } };
      let result = null;
      try { result = await response.json(); } catch { throw new Error(`${provider} HTTP ${response.status}`); }
      if (!response.ok || !result?.ok || !result?.data) throw new Error(result?.error?.message || `${provider} HTTP ${response.status}`);
      return { ...result.data, _health: { updatedAt: result.meta?.updatedAt || new Date().toISOString(), source: result.meta?.cache ? "cache" : "network" } };
    } catch (error) {
  ...
  async function fetchMCLShows(provider, sourceId, date, signal) {
    ...
    try {
      ...
      if (lifecycle.timedOut()) throw new Error(`${labelForProvider(provider)} 場次讀取逾時，請稍後重試。`);
      return { ...result, _health: { updatedAt: result?.source?.updatedAt || new Date().toISOString(), source: result?.source?.cache ? "cache" : "network" } };
    } catch (error) {
      if (lifecycle.timedOut()) throw new Error(`${labelForProvider(provider)} 場次讀取逾時，請稍後重試。`);
      throw error;
    } finally { lifecycle.cleanup(); }
  ...
  function enrichSessionWithVariant(provider, sourceId, session, match) {
    ...
    return { ...session, versionName: [session?.versionName, versionText].filter(Boolean).join(" · "), _phase8cMovieSourceId: sourceId, _phase8cVariantTags: tags };
  }
  ...
  async function fetchProvider(provider, match, date, signal) {
    ...
    if (!successes.length) throw new Error(failures.map(item => item.error).filter(Boolean).join("；") || `${provider} 場次暫不可用`);
    const sessions = successes.flatMap(item => (item.result?.sessions || []).map(session => enrichSessionWithVariant(provider, item.sourceId, session, match)));
    const allSessions = successes.flatMap(item => (item.result?.allSessions || []).map(session => enrichSessionWithVariant(provider, item.sourceId, session, match)));
    ...
    const metadataComplete = typeof completenessPolicy === "function" ? Boolean(completenessPolicy(successes)) : true;
    return {
      ...
      _partialError: failures.length ? `${failures.length}/${sourceIds.length} 個版本來源暫時無法更新` : null
    };
  }
  ...
  function metadataSecondary(session, metadata) {
    const subtitleText = metadata.subtitles?.includes("unknown") ? "字幕未提供" : `字幕：${metadata.subtitleLabels.join("、")}`;
    return [session?.house?.name, ...metadata.formatLabels, ...metadata.languageLabels, subtitleText].filter(Boolean).join(" · ");
  }
  ...
  function normalizeBroadwaySession(session) {
    ...
    const seatText = Number.isFinite(available) ? Number.isFinite(total) ? `${available}/${total} 可選` : `${available} 個可選` : "座位資料暫缺";
    return normalizedBase("broadway", session, available, total, Number.isFinite(session?.price?.display) ? session.price.display : null, session?.bookingUrl || null, seatText, seatClass(available, total));
  }
  function normalizeMCLSession(session) {
    const occupied = session?.seatSummary?.occupiedPercent;
    let seatText = "座位資料稍後提供";
    let klass = "unknown";
    if (Number.isFinite(occupied)) { seatText = `約 ${Math.round(occupied)}% 已售`; klass = occupied >= 90 ? "full" : occupied >= 70 ? "limited" : "available"; }
    const price = Number.isFinite(session?.price?.adult) ? session.price.adult : Number.isFinite(session?.price?.display) ? session.price.display : null;
  ...
  function normalizeEmperorSession(session) {
    ...
    const seatText = Number.isFinite(available) ? Number.isFinite(total) ? `${available}/${total} 未售` : `${available} 未售` : "座位資料暫缺";
    return normalizedBase("emperor", session, available, total, Number.isFinite(session?.price?.display) ? session.price.display : null, session?.bookingUrl || state.match?.emperor?.movie?.bookingUrl || null, seatText, seatClass(available, total));
  }
  function normalizeGenericSession(provider, session) {
    ...
    const price = Number.isFinite(session?.price?.adult) ? session.price.adult : Number.isFinite(session?.price?.display) ? session.price.display : null;
    const seatText = Number.isFinite(available) ? Number.isFinite(total) ? `${available}/${total} 可選` : `${available} 個可選` : "座位資料暫缺";
    return normalizedBase(provider, session, available, total, price, session?.bookingUrl || null, seatText, seatClass(available, total));
  }
  function normalizeSession(provider, session) {
    const normalizer = comparisonAdapter(provider)?.normalizeSession;
    return typeof normalizer === "function" ? normalizer(session) : normalizeGenericSession(provider, session);
  }
  ...
  function providerErrorHtml() {
    const messages = activeProviders().filter(provider => state.errors[provider.key]).map(provider => `${provider.label}：${state.errors[provider.key]}`);
    if (!messages.length) return "";
    return `<div class="provider-compare-warning"><strong>部分版本或院線暫時無法更新</strong>${messages.map(message => `<span>${escapeHtml(message)}</span>`).join("")}<button type="button" data-provider-compare-retry>重新載入比較</button></div>`;
  }
  function renderDates() {
    const dates = combinedDates();
    if (!dates.length) return "";
    const activeCount = activeProviders().length;
    return `<div class="provider-compare-date-rail"><span class="provider-compare-date-label">日期</span><div class="provider-compare-dates" data-provider-count="${activeCount}">${dates.map(date => {
      const availability = dateProviders(date);
      return `<button type="button" class="provider-compare-date ${date === state.selectedDate ? "active" : ""}" data-provider-compare-date="${escapeHtml(date)}"><strong>${escapeHtml(formatDate(date))}</strong><span class="${availability.className}">${escapeHtml(availability.label)}</span></button>`;
    }).join("")}</div></div>`;
  }
  function renderTimelineItem(item) {
    const metadata = item.metadata || sessionMetadata({});
    const capabilities = sharedCore?.showtimeCapabilities?.(item.provider, item) || {
      price: { availability: "unknown" }, seatSummary: { availability: "unknown" }, booking: { availability: item.bookingUrl ? "available" : "unknown" }
    };
    const seatText = capabilities.seatSummary.availability === "unsupported" ? "座位資料不提供" : item.seatText;
    const priceText = capabilities.price.availability === "unsupported" ? "不提供" : Number.isFinite(item.price) ? `$${escapeHtml(item.price)}` : "—";
    const cineArtSeatMap = item.provider === "cineart" && Boolean(item.sourceId) && window.HKCinemaProviderRegistry?.hasCapability?.("cineart", "seatMap");
    const cardAttrs = [
    ...
    const seatAttrs = cineArtSeatMap
      ...
      : "";
    const seatClassName = `${capabilities.seatSummary.availability === "unsupported" ? "unknown" : item.seatClass}${cineArtSeatMap ? " seatmap-launch cineart-seatmap-launch" : ""}`;
    return `
      <article class="provider-compare-show phase6m-show-card phase6o-native-show" ${cardAttrs}>
        ...
        <div class="provider-compare-show-main">
          <div class="provider-compare-show-topline"><span class="provider-compare-source ${escapeHtml(item.provider)}">${escapeHtml(item.providerLabel)}</span><strong>${escapeHtml(item.cinemaName)}</strong></div>
        ...
        <div class="provider-compare-show-actions"><div class="provider-compare-show-price">${priceText}</div>${bookingAction}</div>
      </article>`;
  }
  function renderTimeline() {
    const labels = providerLabels();
    if (state.loadingDate) return `<section class="provider-compare-section"><div class="provider-compare-loading"><strong>正在整理同日場次</strong><span>正在合併 ${escapeHtml(labels.join("、"))} 的所有版本場次...</span></div></section>`;
    ...
    return `<section class="provider-compare-section provider-compare-timeline-section">${renderDates()}<div class="provider-compare-section-heading"><div><p class="eyebrow">${escapeHtml(formatDate(state.selectedDate))}</p><h2>全部場次</h2></div><small>${escapeHtml(counts.join(" · "))} · 共 ${sessions.length} 場</small></div>${sessions.length ? `<div class="provider-compare-timeline">${sessions.map(renderTimelineItem).join("")}</div>` : `<div class="provider-compare-empty"><strong>這一天暫時沒有場次</strong><span>可選擇其他日期，或稍後重新載入。</span></div>`}<details class="provider-compare-note"><summary>票價及座位說明</summary><p>同一電影的語言版、字幕版及特殊放映版本已合併在此；可使用篩選縮窄結果。票價為院線提供的標準／成人價，未知資料不會推測。</p></details></section>`;
  }
  ...
  function render() {
    ...
    content.innerHTML = `<div class="provider-compare-hero">${poster ? `<img src="${escapeHtml(poster)}" alt="${escapeHtml(match.title)}">` : `<div class="provider-compare-poster-placeholder">HK</div>`}<div><p class="eyebrow">MOVIE</p><h1>${escapeHtml(match.title)}</h1><div class="provider-compare-status"><span>${labels.length} 院線</span><small>${aggregate?.variants?.length > 1 ? `${aggregate.variants.length} 個放映版本已合併` : "統一電影場次"}</small></div></div></div>${body}`;
  ...
  function assignProviderResult(key, result, match, hasDate = true) {
    ...
    if (hasDate) { rememberCriteriaDateDecision(key, result, match); state.availableDates[key] = availableDatesFor(key, result, match); }
  }
  async function loadDate(date, cycle = null) {
    ...
    const promises = providers.map(provider => {
      ...
      const reusableSelectedDate = state.data[key]?._requestedDate === date && (typeof reusePolicy !== "function" || reusePolicy(state.data[key]));
    ...
    results.forEach((result, index) => {
      ...
      else { state.data[key] = null; state.errors[key] = errorMessage(result.reason); }
    });
    if (!combinedDates().includes(state.selectedDate)) {
      const preferredDate = firstPreferredDate();
      if (preferredDate) { await loadDate(preferredDate, requestCycle); return; }
      state.selectedDate = null;
    }
    state.loadingDate = false;
    render();
  ...
  window.HKCinemaProviderCompare = {
    ...
    getState() {
      return {
        ...
        availableDates: Object.fromEntries(PROVIDERS.map(provider => [provider.key, [...(state.availableDates[provider.key] || [])]])),
  ...
  document.addEventListener("click", event => {
    ...
    if (openButton) { event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); open(openButton.dataset.compareOpen); return; }
    if (event.target.closest("[data-provider-compare-close]")) { event.preventDefault(); close(); return; }
    const dateButton = event.target.closest("[data-provider-compare-date]");
    if (dateButton) { event.preventDefault(); loadDate(dateButton.dataset.providerCompareDate); return; }
    const retry = event.target.closest("[data-provider-compare-retry]");
    if (retry && state.match) { event.preventDefault(); loadInitial(state.match); }
  }, true);
