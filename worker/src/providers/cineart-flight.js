...
export function decodeCineArtFlightPayload(input) {
  ...
  if (!source.includes("self.__next_f.push")) return source;
  ...
  while ((match = FLIGHT_CHUNK_PATTERN.exec(source))) {
    try {
      ...
      // Ignore malformed/non-string chunks and continue scanning.
    }
  }
...
function findPropsObject(flight, predicate) {
  ...
  while ((cursor = flight.indexOf('{"lng":', cursor)) !== -1) {
    ...
    try {
      ...
      // Flight contains many independent JSON records.
    }
    cursor += 7;
  }
  return null;
