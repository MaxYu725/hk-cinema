...
const RELEASE_DATES = ["2026-08-12", "2026-08-13"];

function releaseShowsPayload(selectedDate) {
  const date = RELEASE_DATES.includes(selectedDate) ? selectedDate : RELEASE_DATES[0];
  return {
    ok: true,
    data: {
      movie: {
        id: RELEASE_MOVIE.id,
        provider: RELEASE_MOVIE.provider,
        sourceId: RELEASE_MOVIE.sourceId,
        title: RELEASE_MOVIE.title,
        releaseDate: RELEASE_MOVIE.releaseDate,
        durationMinutes: RELEASE_MOVIE.durationMinutes,
        rating: RELEASE_MOVIE.rating,
        language: "英文",
        subtitles: ["中文"],
        director: RELEASE_MOVIE.director,
        cast: RELEASE_MOVIE.cast,
        description: null,
        poster: null,
        trailer: null
      },
      availableDates: RELEASE_DATES,
      selectedDate: date,
      sessions: [{
        id: `broadway:release-${date}`,
        provider: "broadway",
        sourceId: `release-${date}`,
        movieId: RELEASE_MOVIE.id,
        cinemaId: "broadway:1",
        cinema: {
          id: "broadway:1",
          provider: "broadway",
...
async function installReleaseUiApiFixture(page) {
  await page.route(`${API_BASE}/**`, async route => {
    ...
    if (url.pathname === "/api/broadway/movies") {
      ...
      payload = releaseShowsPayload(url.searchParams.get("date"));
    }
    ...
    // Release presentation smoke must be deterministic. Provider reachability and
    // M7A-M7D live contracts are enforced by the dedicated candidate-validation gate.
    await route.fulfill({
      status: 503,
      contentType: "application/json",
      body: JSON.stringify({
        ok: false,
        error: { message: "provider unavailable in deterministic release UI fixture" }
      })
    });
  });
}
...
test.beforeEach(async ({ page }) => {
  await installReleaseUiApiFixture(page);
});
